...
void main() {
    ...
    vec3 ambient = strength_ambient * vec3(color);
    vec3 diffuse = strength_diffuse * diff * vec3(color);
    vec3 specular = strength_specular * spec * vec3(color);

    frag_color = vec4(ambient + diffuse + specular, color.a);
}
